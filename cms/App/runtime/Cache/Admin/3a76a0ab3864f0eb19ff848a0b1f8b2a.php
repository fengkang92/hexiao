...
<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-left: 20px"><span class="l" style="margin-right: 10px"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="" href="<?php echo U('Chain/operation');?>"><i class="Hui-iconfont">&#xe600;</i> 添加分店</a></span><a href="<?php echo U('Excels/expChain');?>?status=<?php echo ($status); ?>&create_time=<?php echo ($create_time); ?>"><input class="btn btn-success radius" id="aaa" type="button" value="Excel导出" ></a> <span class="r">共有数据：<strong><?php echo ($num); ?></strong> 条</span></div>
<div style="margin-left: 20px;margin-top: 20px">
    <form class="form form-horizontal" action="" method="post">
        <select name="status" style="height: 30px;width: 80px;text-align: center;">
            ...
            <option value="2" <?php if($status == 2): ?>selected="selected"<?php endif; ?> >禁用</option>
        </select>
...
<div class="page-container">
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tbody>
            <?php if(is_array($chain_data)): $i = 0; $__LIST__ = $chain_data;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    <td><input name="" type="checkbox" value=""></td>
                    <td><?php echo ($v["id"]); ?></td>
                    <td><?php echo ($v["ch_name"]); ?></td>
                    <td class="text-c"><?php echo ($v["adress"]); ?></td>
                    ...
                    <td class="td-manage">
                        <?php if($v["del"] == 1): ?><a style="text-decoration:none" href="javascript:;" title="正常" status="<?php echo ($v["status"]); ?>" chain_id="<?php echo ($v["id"]); ?>" class="check_status"><i class="Hui-iconfont">&#xe631;</i></a>
                        <?php else: ?>
                            <a style="text-decoration:none" href="javascript:;" title="禁用" status="<?php echo ($v["status"]); ?>" chain_id="<?php echo ($v["id"]); ?>" class="check_status"><i class="Hui-iconfont">&#xe6e1;</i></a><?php endif; ?>
                        <a style="text-decoration:none" class="ml-5" onClick=""
                           href="<?php echo U('Chain/operation');?>?id=<?php echo ($v["id"]); ?>" title="修改"><i class="Hui-iconfont">&#xe6df;</i></a>
                    </td>
