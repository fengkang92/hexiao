	...
	public function __construct(){
		...
		if(CONTROLLER_NAME!="Login"){
			...
			if ($admin_name != 'admin') { //超级管理员直接跳过
				...
				$auth_id = M('rbac_role_auth')->where('role_id='.$role_id)->getField('auth_id',true);
				$auth_id = implode(',',$auth_id);
				$auth_infos = M('rbac_auth')->where('id in ('.$auth_id.')')->select();
				foreach ($auth_infos as $key => $v) {
					$auth[] = $v['auth_c'].'/'.$v['auth_a'];
				}
				
				if (in_array($auth_ac, $auth)) {
					return true;
				}else{
					echo "<script>alert('对不起，你没有权限...');</script>";
					exit;
				}
			}else{
				return true;
