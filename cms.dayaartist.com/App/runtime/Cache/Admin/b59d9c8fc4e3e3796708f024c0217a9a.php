...
<div class="page-container">
	<form class="form form-horizontal" action="?id=<?php echo ($id); ?>" method="post" onsubmit="return ac_from();" enctype="multipart/form-data">
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-3">
				<input type="text" class="input-text" placeholder="简介" name="describe" id="describe" value="<?php echo ($video["describe"]); ?>">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-3">
				<input type="text" class="input-text" placeholder="详情" name="content" id="content" value="<?php echo ($video["content"]); ?>">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-3">
				<select class="inp_1" name="cateid" id="cateid" onchange="getcid();" style="width:150px;margin-right:5px;">
					...
					<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if($v["id"] == $video['category_id']): ?>selected="selected"<?php endif; ?>>-- <?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					<!-- 遍历 -->
				</select>
				<br>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-3">
				<input type="text" class="input-text" placeholder="视频链接：" name="main_video_url" id="main_video_url" value="<?php echo ($video["main_video_url"]); ?>">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>时长：</label>
			<div class="formControls col-xs-8 col-sm-3">
				<input type="text" class="input-text" placeholder="时长" name="duration" id="duration" value="<?php echo ($video["duration"]); ?>">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-3">
				...
				<img src="https://api.joyfamliy.com/images_literature<?php echo ($video["main_img_url"]); ?>" width="250" style="margin-bottom: 3px;" />
				<br />
				<?php } ?>
				<input type="file" name="main_img_url" id="main_img_url" />
			</div>
		</div>
